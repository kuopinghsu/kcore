# Spike-based Simulation

This directory contains tools for Spike ISA simulator-based verification.
...
- **rv32_sim.cpp**: Wrapper for running Spike simulator
- **trace_compare.py**: Python script to compare RTL trace with Spike trace
- **analyze_mem_trace.py**: Memory transaction verification script (CPU ↔ AXI memory)
- **parse_call_trace.py**: Call trace analyzer with function profiling and stack analysis

## Usage

### Build Spike simulator wrapper:
```bash
make build-sim
```
...
```bash
make sim             # Run default test (simple)
...
```bash
# Basic usage
python3 sim/parse_call_trace.py <trace_file> <elf_file> <toolchain_prefix> [output_file]

# Example
python3 sim/parse_call_trace.py build/rtl_trace.txt build/test.elf riscv-none-elf- call_report.txt

# With custom output file
python3 sim/parse_call_trace.py build/rtl_trace.txt build/test.elf riscv-none-elf- my_report.txt
```

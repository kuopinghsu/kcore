...
## Memory Trace Analysis (analyze_mem_trace.py)

For memory transaction verification (CPU ↔ AXI memory interface), see [../docs/memory_trace_analysis.md](../docs/memory_trace_analysis.md).

The `analyze_mem_trace.py` script verifies consistency between CPU memory operations and AXI memory transactions.

### Usage

```bash
# Automatic via Makefile
make memtrace                  # Run with memory tracing
make memtrace-<test>           # Run specific test with memory tracing

# Manual verification
python3 scripts/analyze_mem_trace.py build/mem_trace.txt
```

See the full documentation for details on memory trace format and verification features.

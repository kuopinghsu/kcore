...
This directory contains the RISCOF configuration for running RISC-V architectural compliance tests on the kcore processor.
...
├── kcore/                      # DUT plugin for kcore
│   ├── riscof_kcore.py         # Plugin implementation (Verilator)
...
Note: The config.ini file uses relative paths (spike, kcore) which are resolved at runtime by the plugins.
...
The kcore processor implements **RV32IM + Partial A**:
...
- DUT plugin: kcore (Verilator-based RTL)
- Paths to ISA and platform YAML files

### kcore_isa.yaml

Specifies the ISA implementation for kcore:
...
- The kcore processor is **RV32IMA** (no C-extension support)
